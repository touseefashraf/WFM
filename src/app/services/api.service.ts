...
export class ApiService {
    ...
    doUserRedirects(resp: any, router: Router) {
        if (resp.data.user_type !== 'enduser') {
    ...
    isManager(): boolean {
        if (this.isAuthenticated() && this.user.user_type === ConstantsService.USER_ROLES.MANAGER) {
            return true
        } else {
            return false
        }
    ...
    isHr(): boolean {
        if (this.isAuthenticated() && this.user.user_type === ConstantsService.USER_ROLES.HR) {
            return true
        } else {
            return false
        }
    ...
    isEmployee(): boolean {
        if (this.isAuthenticated() && this.user.user_type === ConstantsService.USER_ROLES.EMPLOYEE) {
            return true
        } else {
            return false
        }
